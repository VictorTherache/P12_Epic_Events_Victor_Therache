...
class EventAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.current_user.role == 'sales': 
            my_clients = Client.objects.filter(salesContact= self.current_user.id)
            supports_queryset = CustomUser.objects.filter(role = 'support')            
            current_user = CustomUser.objects.filter(email = self.current_user.email)
            print(self.fields)
            if self.fields:
...
class EventAdminPermissions(admin.ModelAdmin):
    ...
    def get_form(self, request, *args, **kwargs):
         form = super(EventAdminPermissions, self).get_form(request, *args, **kwargs)
         form.current_user = request.user
         return form


    def has_add_permission(self, request):
        return True if request.user.role == 'sales' or request.user.role == 'management' else False
...
class ContratAdminPermissions(admin.ModelAdmin):
    ...
    def get_form(self, request, *args, **kwargs):
         form = super(ContratAdminPermissions, self).get_form(request, *args, **kwargs)
         form.current_user = request.user
         return form
    ...
    def has_change_permission(self, request, obj=None):
        ...
        if request.user.role == 'management':
            return True                           
        # if request.user.role == 'support':
        #     my_clients = Client.objects.filter(supportContact= request.user.id)
...
class ClientAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # access object through self.instance...
        if self.current_user.role == 'management' or  self.current_user.role == 'sales' : 
...
class ClientAdminPermissions(admin.ModelAdmin):
    ...
    def get_form(self, request, *args, **kwargs):
        form = super(ClientAdminPermissions, self).get_form(request, *args, **kwargs)
        form.current_user = request.user
        return form
...
class CustomUserChangeForm(forms.ModelForm):
    """A form for updating CustomUsers. Includes all the fields on
    the CustomUser, but replaces the password field with admin's
    ...
    class Meta:
        model = CustomUser
        fields = ('email', 'password', 'first_name', 'last_name', 'role', 'is_active', 'is_admin')

    def clean_password(self):
        # Regardless of what the CustomUser provides, return the initial value.
...
class CustomUserAdmin(BaseUserAdmin):
    """
    Page d'Admin concernant les utilisateurs, définit les fields à afficher et trier
    """
    ...
    fieldsets = (
        (None, {'fields': ('email', 'password','first_name','last_name', 'is_staff', 'role')}),
...
sales_site = EpicEventAdminArea(name='EpicEvent')
# gestion_site = GestionAdminArea(name='GestionAdmin')
# support_site = SupportAdminArea(name='SupportAdmin')

sales_site.register(Event, EventAdminPermissions)
sales_site.register(CustomUser, CustomUserAdmin)
sales_site.register(Client, ClientAdminPermissions)
sales_site.register(Contrat, ContratAdminPermissions)
