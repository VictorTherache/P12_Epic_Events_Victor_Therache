...
class HasContratChangePermission(permissions.BasePermission):
    ...
    """
    message = 'Vous devez avoir les autorisations nécessaires pour effectuer le changement'
...
class HasContratCreatePermission(permissions.BasePermission):
    ...
    """
    message = 'Vous devez avoir les autorisations nécessaires pour créer un contrat'
...
class HasContratDeletePermission(permissions.BasePermission):
    ...
    """
    message = 'Vous devez avoir les autorisations nécessaires pour créer un utilisateur'
    ...
    def has_permission(self, request, view):
        ...
        if request.user.role == 'sales':
            ...
            my_contrats = Contrat.objects.filter(salesContact = request.user.id)
...
class HasEventCreatePermission(permissions.BasePermission):
    ...
    """
    message = 'Vous devez avoir les autorisations nécessaires pour créer un évènement'

    def has_permission(self, request, view):
        if request.user.role == 'management':
            support_id = CustomUser.objects.get(id=request.data['supportContact'])
            print(support_id)
            return True
        if request.user.role == 'sales':
            support_user = CustomUser.objects.get(id=request.data['supportContact'])
